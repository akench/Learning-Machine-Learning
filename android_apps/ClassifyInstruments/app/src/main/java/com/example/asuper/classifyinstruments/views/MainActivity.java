...
public class MainActivity extends Activity implements View.OnClickListener {
	...
	public void onClick(View view) {
		...
		if (view.getId() == R.id.btn_class) {
			...
			//TEMP TEMP TEMP
			Bitmap testBmp = RotateBitmap(processed_image, 90);
            imageView.setImageBitmap(testBmp);


            int[] tempPix = new int[PIXEL_WIDTH * PIXEL_WIDTH];
            testBmp.getPixels(tempPix, 0, testBmp.getWidth(), 0, 0, testBmp.getWidth(), testBmp.getHeight());

            float[] tempPixels = new float[tempPix.length];

            for(int i = 0; i < tempPix.length; i++) {

                int pix = tempPix[i];
                int b = pix & 0xff;
                tempPixels[i] = (float)((0xff - b) / 255.0);
            }



            final Classification resTemp = myClassifier.recognize(tempPixels);

            if (resTemp.getLabel() == null) {
                text += myClassifier.name() + ": Error\n";
            } else {
                if(resTemp.getLabel().equals("0")){
                    text += "Piano\n";
                }
                else if(resTemp.getLabel().equals("1")){
                    text += "Guitar\n";
                }

                text += ("Probability:" + resTemp.getConf()*100 + "%\n");
            }
            classText.setText(text);
